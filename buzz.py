...
FIXED: Better login handling with fallback
"""
...
class BuzzBot:
    ...
    async def _get_init_data_from_tg(self, phone: str = None, session_str: str = None) -> Optional[str]:
        """Get init_data from Telegram using Pyrogram"""
        try:
            if session_str:
    ...
    async def _pyrogram_login(self, phone: str) -> tuple:
        """Login with phone + code - FIXED with better error handling"""
        ...
        try:
            ...
            # Try to send code first
            try:
                sent = await client.send_code(phone)
                self.info("Code sent! Check your Telegram.")
            except Exception as e:
                if "phone" in str(e).lower():
                    self.err("Invalid phone number format. Use +639XXXXXXXXX")
                    await client.disconnect()
                    os.unlink(session_path)
                    return None, None
                raise e
            ...
            init_data = await self._get_init_data_from_tg(phone, session_str)
            return init_data, session_str

        except Exception as e:
            self.err(f"Login error: {e}")
            await client.disconnect()
            os.unlink(session_path)
            return None, None

    async def _login_with_init_data(self) -> bool:
        """Try to login using init_data from file"""
        if os.path.exists("init_data.txt"):
            try:
                with open("init_data.txt", "r") as f:
                    self.init_data = f.read().strip()
                if self.init_data and len(self.init_data) > 50:
                    self.info("Using saved init_data")
                    return True
            except:
                pass
        return False
    ...
    async def _login(self, phone: str = None) -> bool:
        """Main login flow with fallbacks"""
        
        # First try: Use saved init_data.txt
        if await self._login_with_init_data():
            return True
        
        # Second try: Use saved session
        session_str = None
        ...
        # Third try: Fresh login with phone
        if not phone:
            phone = input(f"{Colors.GREEN}Phone (with country code, e.g., +639XXXXXXXXX): {Colors.RESET}").strip()
        ...
        # Fourth try: Ask user to paste init_data manually
        self.info("Could not auto-login. Please paste init_data manually:")
        self.info("(Open the bot in Telegram, copy the tgWebAppData from URL)")
        manual_init = input(f"{Colors.YELLOW}Paste init_data: {Colors.RESET}").strip()
        if manual_init and len(manual_init) > 50:
            self.init_data = manual_init
            with open("init_data.txt", "w") as f:
                f.write(manual_init)
            return True
    ...
    async def start(self):
        ...
        if not await self._login():
            ...
            self.info("Please make sure you have:")
            self.info("1. A valid phone number with country code (e.g., +639XXXXXXXXX)")
            self.info("2. The Buzzreferralbot is open on Telegram")
            self.info("3. You have internet connection")
            self.info("\nIf login keeps failing, you can manually get init_data:")
            self.info("1. Open the bot in Telegram Web/App")
            self.info("2. Copy the tgWebAppData from the URL")
            self.info("3. Save it as init_data.txt in the script folder")
            return
    ...
    async def _init(self) -> bool:
        ...
        if not self.init_data:
            self.err("No init_data available")
            return False
